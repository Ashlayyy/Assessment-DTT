<template>
  <section v-if="mode == 'create'" class="formSection">
    ...
    <form @submit.prevent="createListing()" class="listing-form">
      ...
      <div>
        <label class="required inputTitle photoUploadText" for="image"
        ...
        <div class="parentDivider">
          <label id="photoInput" class="photoUploadTextTwo input" for="image">
            <div>
              <img
                class="photo_image"
                src="../../assets/icons/DTTIcons/ic_upload@3x.png"
                id="photoUpload"
                alt="Upload icon"
              />
            </div>
          ...
          <img
            v-if="file"
            class="photoRemove"
            src="../../assets/icons/DTTIcons/ic_clear_white@3x.png"
            id="photoRemove"
            alt="Remove Icon"
            @click="removePhoto()"
          />
  ...
  <section
    v-else-if="mode == 'edit' && house && house.location && house.location.street != ''"
    class="formSection"
  >
    ...
    <form @submit.prevent="createListing('edit')" class="listing-form">
      ...
      <div class="shortInputs">
        ...
        <div>
          <label class="required inputTitle" for="addition">Garage</label>
          <select
            v-show="showGarageSelect == true"
            class="short_input input garageInput"
            name="garage"
            id="garageInput"
      ...
      <div>
        ...
        <textarea
          id="descriptionInput"
          class="long_input_textarea input"
...
export default {
  ...
  methods: {
    ...
    checkAll(mode) {
      if (mode == 'edit') {
        this.array = [
          ...
          this.house.location.houseNumber,
          ...
          this.house.rooms.bathrooms,
          this.house.rooms.bedrooms,
    ...
    async sendImage() {
      try {
        if (this.house.id) await API.createImage(this.house.id, this.photoData)
        else if (this.id) await API.createImage(this.id, this.photoData)
    ...
    async sendForm(mode) {
      try {
        ...
        if (this.mode == 'edit' || mode == 'edit') {
          ...
          formData.append(
            'numberAddition',
            this.location.houseNumberAddition != null &&
              this.location.houseNumberAddition != undefined
              ? this.location.houseNumberAddition
              : ' '
          )
